...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_residual = True, num_conv_blocks = 1):
        super().__init__() 
        ...
        #append the final convolutional block with out_channels as the input
        self.convBlocks.append(nn.Sequential(
...
class DownsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels, t_dimensions=128):
        ...
        self.residualBlock = ResidualBlock(in_channels=in_channels, out_channels=out_channels, use_residual=True, num_conv_blocks=1)
...
class UNet(nn.Module):
    def __init__(self, t_dim, device='cpu'):
        ...
        self.input = ResidualBlock(in_channels=self.icc, out_channels=self.icc*self.scale_rate) #2 to 8

        self.d1 = DownsampleBlock(in_channels=self.icc*self.scale_rate, out_channels=self.icc*self.scale_rate**2) #8 to 32 
        self.d2 = DownsampleBlock(in_channels=self.icc*self.scale_rate**2, out_channels=self.icc*self.scale_rate**2) #32 to 32
        ...
        self.u1 = UpsampleBlock(in_channels=(self.icc*self.scale_rate**2)*2, out_channels=(self.icc*self.scale_rate)) #64 to 8, not 32 to 8 because channels are concatenated in downsampling
        self.u2 = UpsampleBlock(in_channels=self.icc*self.scale_rate*2, out_channels=(self.icc*self.scale_rate)) # 16 to 8
    ...
    def position_embeddings(self, t, t_dim_channels):
        #Creates a tensor for the frequency denominator of length channels/2 (step 2) and normalizes it to the number of total embedding channels
    ...
    def forward(self, x, t):
        ...
        t = self.position_embeddings(t, self.t_dim)
